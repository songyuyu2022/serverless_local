...
class MetricsLogger:
    ...
    - 初次写入写 header
    - 后续 append 追加
    - 支持非常多的字段（controller 中使用的字段都能自动加入）
    """
    ...
    def _extract_series(rows: Iterable[Dict[str, str]]) -> Tuple[
        Tuple[list, list], Tuple[list, list], Tuple[list, list]]:
    ...
    def plot_loss_and_acc(csv_path: str, output_path: str = "metrics.png", title: str = "Training Metrics") -> None:
        ...
        Args:
            csv_path: 由 :class:`MetricsLogger` 生成的 CSV 路径。
            output_path: 图片保存路径，默认生成 ``metrics.png``。
            title: 图表标题。
        """
    ...
    def plot_loss_and_acc(csv_path: str, output_path: str = "metrics.png", title: str = "Training Metrics") -> None:
        ...
        if not loss_vals and not acc1_vals and not acc5_vals:
            raise ValueError(f"未在 {csv_path} 中找到 loss 或 acc 字段，无法绘图")
